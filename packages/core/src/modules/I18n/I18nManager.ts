...
export class I18nManager {
  private currentLocale: Language = 'zh-CN';
  ...
  detectLanguage(text: string): { language: Language; confidence: number } {
    ...
    for (const [lang, pattern] of Object.entries(patterns)) {
      if (pattern.test(text)) {
        return { language: lang as Language, confidence: 0.95 };
      }
    }
...
export class GrammarChecker {
  ...
  private async checkWithAI(text: string): Promise<GrammarCheck> {
...
Return a JSON object with:
{
  ...
  private basicCheck(text: string): GrammarCheck {
...
export class SpellChecker {
  ...
  async checkSpelling(text: string): Promise<{ word: string; isCorrect: boolean; suggestions: string[] }[]> {
    const words = text.match(/\b\w+\b/g) || [];
    const results: { word: string; isCorrect: boolean; suggestions: string[] }[] = [];
