...
export const DETECTION_DIMENSIONS = [
...
export class AntiDetectionEngine {
  ...
  private analyzeVocabularyUniformity(text: string): number {
  ...
  private analyzeSentenceLengthVariance(text: string): number {
    ...
    if (variance < 50) return 0.8;
    if (variance < 100) return 0.5;
    if (variance < 200) return 0.2;
    return 0;
  }

  private analyzeParagraphOpeningPattern(text: string): number {
    ...
    if (uniqueOpenings.size < openings.length * 0.3) return 0.7;
    if (uniqueOpenings.size < openings.length * 0.5) return 0.4;
    return 0;
  }

  private analyzeTransitionWordFrequency(text: string): number {
    ...
    if (density > 3) return 0.5;
    if (density > 1) return 0.2;
    return 0;
  }

  private analyzeEmotionalWordDensity(text: string): number {
    let count = 0;
    ...
    if (density < 2) return 0.8;
    if (density < 5) return 0.5;
    if (density < 10) return 0.2;
    return 0;
  }

  private analyzePersonalPronounUsage(text: string): number {
  ...
  private analyzeContradictions(text: string): number {
    ...
    if (contradictions > 5) return 0.6;
    if (contradictions > 2) return 0.3;
    return 0;
  }

  private analyzeSpecificityLevel(text: string): number {
  ...
  private analyzeIdiomUsage(text: string): number {
    const idioms = [
  ...
  private analyzePunctuationPattern(text: string): number {
    const quotes = (text.match(/[""]/g) || []).length;
    const parentheses = (text.match(/[（(][^)]+[）)]/g) || []).length;
    const totalPunctuation = (text.match(/[。！？，、；：""''（）]/g) || []).length;

    if (totalPunctuation === 0) return 0.5;

    const quoteRatio = quotes / totalPunctuation;
    const parenRatio = parentheses / totalPunctuation;

    if (quoteRatio < 0.02 && parenRatio < 0.01) return 0.6;
    return 0;
  }

  private analyzeNarrativePerspective(text: string): number {
    ...
    if (firstPerson > 0 && thirdPerson === 0) return 0.2;
    if (thirdPerson > firstPerson * 3) return 0.4;
    return 0;
  }

  private analyzeTemporalConsistency(text: string): number {
  ...
  private analyzeDetailGranularity(text: string): number {
    const sensoryWords = [
      '看', '听', '闻', '尝', '触摸', '感受',
      '看着', '听到', '闻见', '尝到', '摸着'
    ];
  ...
  private analyzeRhetoricalDevices(text: string): number {
    let count = 0;
    for (const device of this.rhetoricalDevices) {
      if (text.includes(device)) count++;
    }

    const similes = (text.match(/像|似|如|仿佛/g) || []).length;
    const metaphors = (text.match(/是|成为|变成/g) || []).length;

    if (similes + metaphors < 3) return 0.6;
    if (count === 0) return 0.5;
    return 0;
  }

  private analyzeCharacterVoiceConsistency(text: string): number {
    const uniqueSpeechMarkers = new Set([
      ...text.match(/[""][^""]+[""]/g) || []
    ]);

    if (uniqueSpeechMarkers.size < 2) return 0.4;
    return 0;
  }

  private analyzeWorldKnowledgeConsistency(text: string): number {
  ...
  private analyzePlotLogicConsistency(text: string): number {
  ...
  private analyzeInformationBalance(text: string): number {
    const paragraphs = text.split(/\n\n+/);
    ...
    for (const len of lengths) {
      if (Math.abs(len - avgLength) > avgLength * 0.8) unbalanced++;
    }

    if (unbalanced > paragraphs.length * 0.5) return 0.6;
    return 0;
  }

  private analyzeUnnecessaryPreambles(text: string): number {
    const preambles = [
      '值得注意的是', '毫无疑问', '必须指出',
      '应当指出', '可以认为', '一般来说'
    ];
    ...
    for (const preamble of preambles) {
      if (text.includes(preamble)) count++;
    }
  ...
  private analyzeFormulaicExpressions(text: string): number {
    let count = 0;
  ...
  private analyzeSuperlativeUsage(text: string): number {
    ...
    if (ratio > 8) return 0.7;
    if (ratio > 4) return 0.4;
    return 0;
  }

  private analyzeHedgingFrequency(text: string): number {
    let count = 0;
  ...
  private analyzePassiveVoiceRatio(text: string): number {
    let count = 0;
  ...
  private analyzeComplexSentenceRatio(text: string): number {
  ...
  private analyzeListStructureFrequency(text: string): number {
    const listPatterns = [
      /第一[、，].*第二[、，].*第三/,
      /首先[、，].*其次[、，].*最后/,
      /一方面[、，].*另一方面/
  ...
  private analyzeTopicSentencePattern(text: string): number {
    const paragraphs = text.split(/\n\n+/);
    ...
    for (const para of paragraphs) {
      ...
      for (const pattern of topicSentencePatterns) {
        if (pattern.test(firstLine)) count++;
      }
    }

    if (count > paragraphs.length * 0.4) return 0.6;
    return 0;
  }

  private analyzeDefinitionIntroduction(text: string): number {
  ...
  private analyzeSummarizationFrequency(text: string): number {
  ...
  private analyzeExampleProvision(text: string): number {
  ...
  private analyzeRhetoricalQuestions(text: string): number {
  ...
  private analyzeSuperfluousEmphasis(text: string): number {
    const emphasisWords = [
      '重要', '关键', '核心', '本质',
      '主要', '基本', '根本', '至关重要'
    ];
  ...
  private analyzeReferenceClarity(text: string): number {
  ...
  private analyzeCoherenceMarkers(text: string): number {
    const markers = [
      '然而', '但是', '因此', '所以', '不过',
      '与此同时', '于是', '接着', '随后', '于是乎'
    ];
  ...
  private generateSuggestions(indicators: AIIndicator[], scores: Record<string, number>): string[] {
...
请直接输出修改后的文本：`;
  ...
  private calculateReadabilityScore(text: string): number {
    ...
    if (avgLength < 30) return 0.8;
    if (avgLength < 50) return 0.6;
    if (avgLength < 80) return 0.4;
    return 0.2;
  }
  ...
  private calculateCoherenceScore(text: string): number {
    ...
    const paragraphs = text.split(/\n\n+/).length;
    if (paragraphs === 0) return 0.5;

    const markerDensity = markerCount / paragraphs;

    if (markerDensity > 2 && markerDensity < 5) return 0.7;
    if (markerDensity >= 5) return 0.4;
    return 0.6;
  }
}
