...
import { NovelProject, TruthFiles, Chapter, AuditResult, ChapterStatus } from '../../types';
...
export abstract class BaseAgent {
  ...
  protected responseCache: Map<string, { result: any; timestamp: Date }> = new Map();
  protected static CACHE_EXPIRY_MS = 30 * 60 * 1000; // 30分钟
  protected static maxConcurrentTasks: number = 3;
  protected static currentConcurrentTasks: number = 0;
  ...
  protected trackTokenUsage(taskType: string, inputTokens: number, outputTokens: number, cost: number = 0): void {
  ...
  protected truncateContent(content: string, maxLength: number = 2000): string {
    if (content.length <= maxLength) return content;
    return content.substring(0, maxLength) + '...[内容已截断]';
  }

  protected optimizePrompt(prompt: string): string {
    if (!this.enableTokenSaving) return prompt;
    return prompt
      .replace(/\s+/g, ' ') // 压缩多个空格
      .replace(/\n\s*\n/g, '\n') // 压缩多个换行
      .trim();
  ...
  protected async acquireTaskSlot(): Promise<void> {
...
export class OnlineArchitectAgent extends BaseAgent {
  getAgentName(): string {
  ...
  async analyzeMarket(project: NovelProject): Promise<any> {
    ...
    const prompt = this.optimizePrompt(
  ...
  async planNovelStructure(project: NovelProject): Promise<any> {
    ...
    let structure: any;
    try { structure = typeof response === 'string' ? JSON.parse(response) : response; } 
    catch { structure = { raw: response }; }
    return { structure };
  }

  async worldBuilding(data: { project: NovelProject; depth?: string }): Promise<any> {
  ...
  async characterDesign(data: { project: NovelProject; count?: number }): Promise<any> {
  ...
  async plotPlanning(data: { project: NovelProject; chapters?: number }): Promise<any> {
  ...
  async outlineGeneration(data: { project: NovelProject }): Promise<any> {
...
export class OnlineWriterAgent extends BaseAgent {
  getAgentName(): string {
    return 'OnlineWriter';
  }
  ...
  async writeChapter(data: { project: NovelProject; chapterNumber: number; outline?: string; truthFiles?: TruthFiles }): Promise<any> {
  ...
  async continueStory(data: { project: NovelProject; lastContent: string; additionalChapters: number }): Promise<any> {
  ...
  async writeScene(data: { project: NovelProject; location: string; characters: string[]; action: string }): Promise<any> {
  ...
  async writeFanfiction(data: { project: NovelProject; premise: string }): Promise<any> {
  ...
  async writeSideStory(data: { project: NovelProject; title: string; viewpoint: string }): Promise<any> {
  ...
  async writeMultiPOV(data: { project: NovelProject; chapterNumber: number; viewpoints: string[] }): Promise<any> {
...
export class OnlineAuditorAgent extends BaseAgent {
  constructor(config: AgentConfig) {
    super(config);
  }
  ...
  private basicAudit(content: string): AuditResult {
    const issues: any[] = [];
  ...
  async auditConsistency(data: { content: string; truthFiles: TruthFiles }): Promise<any> {
  ...
  async generateQualityReport(data: { content: string; chapterNumber?: number }): Promise<any> {
  ...
  async plagiarismCheck(data: { content: string }): Promise<any> {
    this.requireOnline('plagiarism_check');
    return { unique: true, similarity: 0 };
  }

  async styleConsistency(data: { content: string; styleGuide?: string }): Promise<any> {
  ...
  async plotHoleDetection(data: { content: string; plotOutline?: string }): Promise<any> {
...
export class OnlineReviserAgent extends BaseAgent {
  getAgentName(): string {
  ...
  async reviseChapter(data: { content: string; feedback?: string }): Promise<any> {
  ...
  async fixIssues(data: { content: string; issues: string[] }): Promise<any> {
  ...
  async enhanceProse(data: { content: string }): Promise<any> {
  ...
  async rewrite(data: { content: string; style?: string }): Promise<any> {
  ...
  async simplify(data: { content: string }): Promise<any> {
  ...
  async expand(data: { content: string; targetLength?: number }): Promise<any> {
...
export class OnlineStyleEngineerAgent extends BaseAgent {
  getAgentName(): string {
  ...
  async analyzeStyle(data: { content: string }): Promise<any> {
  ...
  async transferStyle(data: { content: string; targetStyle: string }): Promise<any> {
  ...
  async optimizeProse(data: { content: string }): Promise<any> {
  ...
  async changeTone(data: { content: string; tone: string }): Promise<any> {
  ...
  async addMood(data: { content: string; mood: string }): Promise<any> {
  ...
  async formatDialogue(data: { content: string }): Promise<any> {
...
export class OnlineRadarAgent extends BaseAgent {
  getAgentName(): string {
    return 'OnlineRadar';
  }
  ...
  async trackTrends(data: { project: NovelProject }): Promise<any> {
  ...
  async detectRisk(data: { content: string }): Promise<any> {
  ...
  async trackConsistency(data: { content: string; previousContent?: string }): Promise<any> {
    return { consistencyStatus: '一致', issues: [] };
  }

  async analyzeMarket(data: { genre: string }): Promise<any> {
    this.requireOnline('analyze_market');
    const prompt = this.optimizePrompt(
      `## 市场分析\n题材：${data.genre}\n请分析当前市场情况。`
    );
  ...
  async suggestImprovement(data: { content: string; project?: NovelProject }): Promise<any> {
...
export class OfflineArchitectAgent extends BaseAgent {
  getAgentName(): string {
  ...
  private getStructureTemplates(genre: string): any[] {
    return [
      {
        name: '经典四幕式',
  ...
  private generateParts(template: any, totalChapters: number): any[] {
    return template.parts.map((part: any, index: number) => {
      const chapterCount = Math.max(1, Math.floor(totalChapters * part.percent));
      const startChapter = index === 0 ? 1 : 
        template.parts.slice(0, index).reduce((sum: number, p: any) => sum + Math.floor(totalChapters * p.percent), 0) + 1;
  ...
  private generateKeyEventsForPart(partName: string, partIndex: number): string[] {
  ...
  private generateGeography(genre: string, depth: string): any {
    const geographies: Record<string, any> = {
      '玄幻': {
  ...
  private generateSociety(genre: string, depth: string): any {
    return {
  ...
  private generateFactions(genre: string): any[] {
  ...
  private generateHierarchy(genre: string): string {
    const hierarchies = {
      '玄幻': '境界等级森严，强者为尊',
      '都市': '社会阶级分明，财富和权力为主',
      '仙侠': '修仙等级明确，实力决定地位',
  ...
  private generateOrganizations(genre: string): any[] {
    return [
  ...
  private generateCulture(genre: string, depth: string): any {
    return {
  ...
  private generateValues(genre: string): string[] {
    const values = {
  ...
  private generatePowerSystem(genre: string, depth: string): any {
    const powerSystems = {
      '玄幻': {
        systemName: '修炼体系',
  ...
  private generateConflicts(genre: string): any[] {
    return [
  ...
  private getArchetypesForGenre(genre: string): any[] {
  ...
  private generateCharacterName(archetype: any, projectTitle: string): string {
  ...
  private generateAppearance(archetype: any): string {
    const appearances = {
  ...
  private generatePersonality(archetype: any): string[] {
  ...
  private generateBackground(archetype: any): string {
    const backgrounds = {
  ...
  private generateGoals(archetype: any): string[] {
    return [
      '主要目标：实现人生理想/完成使命',
      '短期目标：解决当前问题，提升实力',
      '长期目标：实现最终理想，解开自身谜团'
  ...
  private generateCharacterConflicts(archetype: any): string[] {
    return [
      '内在冲突：内心的挣扎与选择',
      '外在冲突：与敌人的对抗',
      '关系冲突：与重要人物的关系变化'
  ...
  private generateRelationships(archetype: any, index: number): any[] {
    const relationships = [];
    if (index > 0) {
  ...
  private generateCharacterArc(archetype: any): string {
    const arcs = {
  ...
  private generateAbilities(archetype: any, genre: string): any[] {
    const abilities = [
  ...
  private generateMainPlotline(totalChapters: number): any[] {
    const milestones = [
  ...
  private generateKeyPlotPoints(totalChapters: number): any[] {
    const points = [];
    for (let i = 1; i <= 10; i++) {
  ...
  private generateForeshadowing(): any[] {
    return [
  ...
  async themeGeneration(data: { project: NovelProject }): Promise<any> {
    const themes = [
  ...
  async generateDirection(data: { project: NovelProject }): Promise<any> {
    const directions = [
  ...
  private generateChapterTitle(chapter: number, total: number): string {
    const progress = chapter / total;
    if (progress < 0.2) return `第${chapter}章：${this.getEarlyTitle(chapter)}`;
    if (progress < 0.5) return `第${chapter}章：${this.getMiddleTitle(chapter)}`;
    if (progress < 0.8) return `第${chapter}章：${this.getLateTitle(chapter)}`;
    return `第${chapter}章：${this.getFinalTitle(chapter)}`;
  }

  private getEarlyTitle(chapter: number): string {
    const titles = ['初入江湖', '命运的开始', '踏上征程', '初识风云', '第一个任务',
                    '机缘巧合', '拜师学艺', '小试牛刀', '初遇对手', '一鸣惊人'];
    return titles[(chapter - 1) % titles.length];
  }

  private getMiddleTitle(chapter: number): string {
    const titles = ['历练成长', '结识伙伴', '遭遇挑战', '揭开秘密', '实力提升',
                    '风云际会', '暗流涌动', '危机初现', '重要转折', '真相渐露'];
    return titles[(chapter - 1) % titles.length];
  }

  private getLateTitle(chapter: number): string {
    const titles = ['阴谋浮现', '危机四伏', '生死之战', '真相大白', '艰难抉择',
                    '全面冲突', '步步紧逼', '绝境逢生', '终极布局', '最后准备'];
    return titles[(chapter - 1) % titles.length];
  }

  private getFinalTitle(chapter: number): string {
    const titles = ['最终决战', '巅峰对决', '尘埃落定', '新的开始', '尾声',
                    '曲终人散', '前路漫漫', '后日谈', '新的征程', '全书完'];
    return titles[(chapter - 1) % titles.length];
  }

  private generateChapterSummary(chapter: number, total: number): string {
    const progress = chapter / total;
    if (progress < 0.2) return '主角初出茅庐，铺垫世界观，遇到第一个关键人物或事件';
    if (progress < 0.5) return '主角获得成长，剧情发展，新的人物/线索出现';
    if (progress < 0.8) return '冲突升级，剧情走向高潮，主角面临重要选择';
    return '最终决战，故事迎来结局，人物命运交代';
  }

  private generateChapterEvents(chapter: number, total: number): string[] {
    return [
      '主要事件1',
      '主要事件2',
      '人物互动',
  ...
  private getChapterFocus(chapter: number, total: number): string {
    const focuses = ['主角', '重要配角', '反派', '关系发展', '世界观展示', '战斗', '感情'];
    return focuses[chapter % focuses.length];
  }

  private getChapterForeshadowing(chapter: number, total: number): string {
    if (chapter % 5 === 0) {
      return '本章埋下伏笔';
    }
    return '';
  ...
  private getChapterMood(chapter: number, total: number): string {
    const progress = chapter / total;
    if (progress < 0.2) return '轻松/铺垫';
    if (progress < 0.5) return '发展/探索';
    if (progress < 0.8) return '紧张/高潮';
    return '高潮/结局';
  }
}

export class OfflineWriterAgent extends BaseAgent {
  getAgentName(): string {
  ...
  async writeChapter(data: { project: NovelProject; chapterNumber: number; outline?: string; sceneType?: string }): Promise<any> {
...
- 营造相应的氛围`;
  ...
  private getChapterTemplate(genre: string, chapterNumber: number): any {
    const progress = chapterNumber % 10;
    const chapterPhase = progress < 3 ? 'opening' : progress < 7 ? 'development' : 'climax';
    
    const templates = {
      opening: {
        header: (title: string, num: number) => `第 ${num} 章 ${this.generateChapterTitle(num)}`,
  ...
  private getSceneDescription(sceneType: string): string {
  ...
  private getCharacterIntroductionTemplate(): string {
    return `【主要人物】
- [主角名]：[状态/心情描写]
- [配角名]：[状态/目的描写]
- [其他人物]：[简要介绍]`;
  ...
  private getDialogueTemplate(): string {
    return `【对话提示】
- 每个人物的说话风格要一致
- 通过对话展现性格和关系
- 对话要推动情节发展
...
"[台词3]" [人物A] [进一步动作]，"[台词4]"`;
  ...
  private generateChapterTitle(chapter: number): string {
    const titles = ['启程', '相遇', '挑战', '成长', '转折', '危机', '揭秘', '激战', '突破', '新生',
                   '暗流', '迷雾', '真相', '抉择', '守护', '重逢', '绝处', '逢生', '归途', '新章'];
    return titles[(chapter - 1) % titles.length];
  }

  async writeScene(data: { project: NovelProject; location: string; characters: string[]; action: string; mood?: string }): Promise<any> {
...
（描写场景结束时的状态，引出下一个情节或留下悬念）`;
  ...
  async generateDialogue(data: { characters: string[]; topic: string; relationship?: string; mood?: string }): Promise<any> {
...
- 推动情节发展或展现人物`;
  ...
  async generateDescription(data: { target: string; type: string; details?: string }): Promise<any> {
...
战后：[结果、影响、人物状态]`
  ...
  async writeSideStory(data: { project: NovelProject; character: string; theme?: string }): Promise<any> {
...
- 可以埋下与正篇相关的伏笔`;
  ...
  async writeMultiPOV(data: { project: NovelProject; viewpoints: string[]; event: string }): Promise<any> {
...
- 最后汇总时要能串联起来`;
  ...
  async generateSuspense(data: { project: NovelProject; type: string; setup?: string }): Promise<any> {
    const suspenseTypes = {
...
疑问3：`;
  ...
  async writeActionScene(data: { project: NovelProject; combatants: string[]; location: string; stakes?: string }): Promise<any> {
...
- 战斗要服务于人物和情节`;
  ...
  async writeRomanticScene(data: { project: NovelProject; characters: string[]; stage?: string }): Promise<any> {
...
export class OfflineAuditorAgent extends BaseAgent {
  constructor(config: AgentConfig) {
    super(config);
  }
  ...
  private enhancedAudit(content: string): AuditResult {
  ...
  async checkStructure(data: { content: string; outline?: string }): Promise<any> {
    const structure = {
  ...
  async checkCharacters(data: { content: string; characters?: string[] }): Promise<any> {
  ...
  async checkPlotLogic(data: { content: string }): Promise<any> {
    return {
      suggestions: [
        '检查情节发展是否合理',
        '确认没有明显的逻辑漏洞',
  ...
  async checkPacing(data: { content: string }): Promise<any> {
    const wordCount = content.length;
    return {
...
export class OfflineReviserAgent extends BaseAgent {
  getAgentName(): string {
  ...
  async formatText(data: { content: string }): Promise<any> {
  ...
  async suggestRevisions(data: { content: string }): Promise<any> {
    return {
      suggestions: [
        '检查是否有错别字和语病',
        '检查语句是否通顺',
  ...
  async suggestExpansions(data: { content: string; position?: string }): Promise<any> {
    return {
      expansionPoints: [
  ...
  async suggestImprovements(data: { content: string }): Promise<any> {
    return {
      aspects: [
  ...
  async fixCommonIssues(data: { content: string }): Promise<any> {
...
export class OfflineStyleEngineerAgent extends BaseAgent {
  getAgentName(): string {
  ...
  async formatParagraphs(data: { content: string; maxLength?: number }): Promise<any> {
    ...
    for (let para of paragraphs) {
      ...
      if (para) formattedParagraphs.push(para);
    }
    return { formattedContent: formattedParagraphs.join('\n\n') };
  }

  async applyStyleTemplate(data: { content: string; style: string }): Promise<any> {
    ...
    return {
      ...
      suggestions: styleTemplates[data.style as keyof typeof styleTemplates] || '保持当前风格',
  ...
  async suggestLiteraryDevices(data: { content: string; sceneType?: string }): Promise<any> {
    const devices = [
  ...
  async suggestVocabulary(data: { content: string; type?: string }): Promise<any> {
    const vocabTypes = {
    ...
    return {
      ...
      examples: vocabTypes[data.type as keyof typeof vocabTypes] || [],
      tip: '根据语境选择最贴切的词汇，避免重复'
    };
  }
}

export class OfflineRadarAgent extends BaseAgent {
  getAgentName(): string {
    return 'OfflineRadar';
  }
  ...
  private extractProtagonist(content: string): string | null {
    const matches = content.match(/【主角】\s*([\u4e00-\u9fa5]{2,4})/);
    return matches ? matches[1] : null;
  }

  async checkLength(data: { content: string; chapterNumber: number }): Promise<any> {
  ...
  private getLengthSuggestion(status: string): string {
  ...
  async trackProgress(data: { project: NovelProject; chapters: Chapter[] }): Promise<any> {
  ...
  private getMilestones(progress: number): string[] {
    const milestones: string[] = [];
    if (progress >= 25) milestones.push('¼ 完成');
    if (progress >= 50) milestones.push('½ 完成');
    if (progress >= 75) milestones.push('¾ 完成');
    if (progress >= 100) milestones.push('全部完成！');
    return milestones;
  }

  async generateChecklist(data: { stage: string }): Promise<any> {
  ...
  async analyzeStructure(data: { content: string }): Promise<any> {
    const wordCount = data.content.length;
    const paragraphs = data.content.split('\n\n').filter(p => p.trim());
    return {
      wordCount,
...
export class AgentSystem {
  ...
  async executeArchitectTask(project: NovelProject, task: 'world_building' | 'character_design' | 'plot_planning' | 'outline_generation', params?: Record<string, any>): Promise<any> {
  ...
  async executeWriterTask(project: NovelProject, chapterNumber: number, options?: { outline?: string; guidance?: string }): Promise<any> {
  ...
  async executeAuditorTask(content: string, truthFiles: TruthFiles, options?: { autoFix?: boolean }): Promise<any> {
  ...
  async executePipeline(project: NovelProject, chapterNumber: number): Promise<any> {
...
export interface PipelineResult {
  success: boolean;
  steps: Record<string, { success: boolean; result?: any; error?: any }>;
...
export class AgentCoordinator {
  ...
  private autoSwitchMode: boolean;
  ...
  private initializeAgents(config: AgentCoordinatorConfig): void {
  ...
  sendMessage(fromAgent: string, toAgent: string, content: string, type: AgentMessage['type'] = 'request', metadata?: Record<string, any>): void {
    const message: AgentMessage = {
  ...
  async executeTask(agentType: string, task: Omit<AgentTask, 'id' | 'createdAt'>, retryCount: number = 0): Promise<{ taskId: string; result: any }> {
    const agent = this.agents[agentType];
    if (!agent) {
    ...
    try {
      ...
      const fullTask: AgentTask = { ...task, id: taskId, createdAt: new Date() };
  ...
  async executePipeline(project: NovelProject, chapterNumber: number, truthFiles?: TruthFiles): Promise<PipelineResult> {
    const startTime = Date.now();
    const results: Record<string, any> = {};
    const steps: Record<string, { success: boolean; result?: any; error?: any }> = {};
  ...
  async executeCustomPipeline(steps: PipelineStep[]): Promise<PipelineResult> {
    const startTime = Date.now();
    const results: Record<string, any> = {};
    const stepResults: Record<string, { success: boolean; result?: any; error?: any }> = {};
  ...
  canExecuteTask(agentType: string, taskType: string): { supported: boolean; requiresOnline: boolean } {
