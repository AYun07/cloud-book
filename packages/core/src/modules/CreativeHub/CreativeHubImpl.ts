...
export class CreativeHub {
  ...
  private config: CreativeHubConfig;
  private sessions: Map<string, CreativeSession> = new Map();
  private documentCache: Map<string, CreativeDocument> = new Map();
  ...
  async createSession(projectId: string): Promise<CreativeSession> {
  ...
  async getSession(sessionId: string): Promise<CreativeSession | null> {
    ...
    try {
      ...
      const session = JSON.parse(content) as CreativeSession;
      this.sessions.set(sessionId, session);
      return session;
    } catch {
      return null;
  ...
  async addDocument(sessionId: string, document: CreativeDocument): Promise<void> {
    try {
      const session = await this.getSession(sessionId);
      if (!session) {
        throw new CloudBookError('Session not found', 'SESSION_NOT_FOUND');
      }

      const embedding = this.vectorizer.embed(document.content);
      const processedDoc: CreativeDocument = {
        ...document,
        embedding,
        metadata: {
          ...document.metadata,
      ...
      throw handleError(error, 'CreativeHub.addDocument');
    }
  }

  async search(sessionId: string, query: string, limit: number = 5): Promise<SearchResult[]> {
    try {
      const session = await this.getSession(sessionId);
      if (!session) {
        throw new CloudBookError('Session not found', 'SESSION_NOT_FOUND');
      ...
      throw handleError(error, 'CreativeHub.search');
    }
  }

  private extractMatchedSegments(content: string, query: string, contextChars: number = 50): string[] {
  ...
  async sendMessage(sessionId: string, message: string, useRAG: boolean = true): Promise<{ response: string; context: CreativeDocument[] }> {
    try {
      const session = await this.getSession(sessionId);
      if (!session) {
        throw new CloudBookError('Session not found', 'SESSION_NOT_FOUND');
      ...
      throw handleError(error, 'CreativeHub.sendMessage');
    }
  }

  private generateContextualResponse(message: string, context: CreativeDocument[]): string {
    if (context.length === 0) {
  ...
  async clearSession(sessionId: string): Promise<void> {
    try {
      ...
      throw handleError(error, 'CreativeHub.clearSession');
    }
  }
  ...
  async deleteSession(sessionId: string): Promise<void> {
    try {
      ...
      throw handleError(error, 'CreativeHub.deleteSession');
    }
  }
  ...
  private async saveSession(session: CreativeSession): Promise<void> {
    await this.storage.write(`sessions/${session.sessionId}.json`, JSON.stringify(session));
  }

  private async saveDocument(document: CreativeDocument): Promise<void> {
