...
const defaultAntiDetectionConfig = {
    enabled: true,
    intensity: 5,
    replaceAIWords: true,
...
const defaultAuditConfig = { dimensions: [], threshold: 0.7, autoFix: true, maxIterations: 3 };
function createCloudBookConfig(llmConfigs = []) {
    return {
        llmConfigs,
        modelRoutes: [],
...
program
    ...
    try {
        const project = await cloudBook.createProject(answers.title, genreMap[answers.genre] || 'fantasy', modeMap[answers.mode]);
...
program
    ...
    try {
        ...
        const chapterResult = await cloudBook.generateChapter(projectId, parseInt(chapter), writingOptions);
        console.log(chalk.green(`\n✅ 第${chapter}章生成完成!`));
...
program
    ...
    const existingIndex = config.llmConfigs.findIndex((c) => c.name === newModelConfig.name);
    if (existingIndex >= 0) {
