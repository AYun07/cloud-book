...
请开始写作：`;
  ...
  private buildReviserPrompt(content: string, issues: any[], truthFiles: TruthFiles): string {
    return `请根据以下问题修订章节内容：

需要修复的问题：
${issues.map((i: any, idx: number) => `${idx + 1}. [${i.type}] ${i.description} - 建议：${i.suggestion}`).join('\n')}
