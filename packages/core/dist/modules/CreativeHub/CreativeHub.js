...
class CreativeHub {
    ...
    index = new Map();
    llmProvider = null;
    constructor(embeddingConfig, chunkConfig, vectorStoreConfig) {
...
请用中文回答。`;
    ...
    async deleteDocument(id) {
        const toDelete = this.documents.filter(d => d.id === id || d.metadata?.parentId === id);
        for (const doc of toDelete) {
            this.index.delete(doc.id);
        }
        this.documents = this.documents.filter(d => d.id !== id && d.metadata?.parentId !== id);
    }
    /**
     * 获取统计信息
     */
