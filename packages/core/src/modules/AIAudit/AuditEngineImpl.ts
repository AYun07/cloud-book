...
export class AuditEngine {
  ...
  private async auditDimension(content: string, dimension: string, options: any): Promise<AuditDimension> {
    ...
    const checkMethods: Record<string, () => { score: number; status: 'pass' | 'warning' | 'fail'; details?: string }> = {
    ...
    let { score, status, details } = checkMethods[dimension]
      ...
      : { score: 70, status: 'warning' as const, details: '未实现的审计维度' };
  ...
  private checkGrammar(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    for (const pattern of subjectVerbMismatches) {
      ...
      if (matches) issues.push(`发现主谓不一致: ${matches.length}处`);
    }
    ...
    else if (score < 80 || issues.length > 2) status = 'warning';
  ...
  private checkSpelling(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    ...
    let status: 'pass' | 'warning' | 'fail' = 'pass';
    if (score < 60) status = 'fail';
    else if (score < 85 || errors.length > 0) status = 'warning';
  ...
  private checkPunctuation(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    let score = 100;
    if (issues.length > 3) score -= 20;
    else if (issues.length > 0) score -= issues.length * 5;
  ...
  private checkSentenceStructure(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    ...
    let score = 80;
    if (issues.length > 0) score -= issues.length * 8;
    if (avgLength > 80) score -= 10;
    if (avgLength < 5) score -= 15;
    if (variation < 0.05) score -= 10;
  ...
  private checkParagraphStructure(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    ...
    let score = 80;
    if (issues.length > 0) score -= issues.length * 6;
    if (avgLength < 20) score -= 10;
    if (avgLength > 180) score -= 10;
  ...
  private checkCoherence(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    const paragraphs = content.split(/\n\n+/).filter(p => p.trim().length > 0);
    ...
    const sentenceTransition = paragraphs.map(p => {
      ...
      for (const tp of transitions) {
        ...
        if (matches) tCount += matches.length;
      }
      return tCount;
    });
    ...
    let score = 80;
    if (issues.length > 0) score -= issues.length * 7;
    if (transitionCount < 2 && paragraphs.length > 2) score -= 15;
  ...
  private checkConsistency(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    
    const characterNames = this.extractCharacterNames(content);
    const nameMentions: Record<string, number> = {};
    for (const name of characterNames) {
      nameMentions[name] = (nameMentions[name] || 0) + 1;
    }
    
    const inconsistentlyUsedNames: string[] = [];
  ...
  private checkCharacterVoice(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    const uniqueSpeechPatterns: string[] = [];
    const speechLengths: number[] = [];
  ...
  private checkDialogue(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    const dialogues = this.extractDialogues(content);
    const paragraphs = content.split(/\n\n+/).filter(p => p.trim().length > 0);
    ...
    let score = 80;
    if (issues.length > 0) score -= issues.length * 8;
    if (dialogues.length < 3) score -= 10;
  ...
  private checkNarrativeVoice(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    let voiceType = '第三人称';
    if (firstPersonRatio > 0.6) voiceType = '第一人称';
    else if (secondPersonRatio > 0.3) voiceType = '第二人称';
  ...
  private checkPacing(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    const paragraphs = content.split(/\n\n+/).filter(p => p.trim().length > 0);
    ...
    let score = 80;
    if (issues.length > 0) score -= issues.length * 7;
    if (avgLength < 20 || avgLength > 200) score -= 10;
  ...
  private checkDescription(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    for (const pattern of adjectivePatterns) {
      const matches = content.match(pattern);
      if (matches) totalAdjectives += matches.length;
    }
  ...
  private checkShowDontTell(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    for (const { pattern } of showPatterns) {
      const matches = content.match(pattern);
      if (matches) showCount += matches.length;
    }
  ...
  private checkEmotionalImpact(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    for (const pattern of bodyLanguagePatterns) {
      ...
      if (matches) bodyLanguageCount += matches.length;
    }
    ...
    let score = 70;
    if (issues.length > 0) score -= issues.length * 8;
    if (emotionalPunctuation > 10) score += 5;
    if (emotionDensity > 2 && emotionDensity < 8) score += 5;
  ...
  private checkConflict(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
  ...
  private checkTension(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    let score = 70;
    if (issues.length > 0) score -= issues.length * 8;
    if (totalTension > 3) score += 5;
    if (cliffhangers.length > 0) score += 5;
  ...
  private checkResolution(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
  ...
  private checkPlotHoles(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
  ...
  private checkTimeline(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    
    const timeExpressions = content.match(
      /[一二三四五六七八九十百千0-9]+[年月日天周分钟秒]|昨天|今天|明天|去年|今年|明年|刚才|不久|很快|突然|随后|之前|之后|现在|将来|过去|早晨|中午|晚上|凌晨|深夜|黄昏|傍晚/g
    ) || [];
    ...
    const timeJumps = content.match(
      /(就在这时|突然|一转眼|眨眼间|刹那间)[^，]+，[^\s，。]+却没有/g
    ) || [];
  ...
  private checkWorldbuilding(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    const rulesPatterns = content.match(
      /(根据|遵循|按照|必须|不能|禁止)[^。]+(规则|设定|规律|法则)/g
    ) || [];
    ...
    let score = 70;
    if (issues.length > 0) score -= issues.length * 8;
    if (totalWorldElements > 5) score += 5;
  ...
  private checkMotivation(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
  ...
  private checkStakes(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    for (const phrase of highStakesIndicators) {
      const matches = content.match(new RegExp(phrase, 'g'));
      if (matches) highStakesCount += matches.length;
    }
    ...
    const consequencePatterns = content.match(
      /(如果|要是|万一)[^，]+，[^\s，。]+就[^\s，。]+(失去|失去|毁灭|死亡|消失)/g
    ) || [];
    ...
    let score = 70;
    if (issues.length > 0) score -= issues.length * 8;
    if (stakesLevelsPresent >= 2) score += 5;
    if (highStakesCount > 0) score += 5;
  ...
  private checkThemes(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    let score = 70;
    if (issues.length > 0) score -= issues.length * 8;
    if (themesPresent.length >= 2) score += 5;
  ...
  private checkSymbolism(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
  ...
  private checkProse(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
  ...
  private checkReadability(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
  ...
  private checkAuthenticity(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    const actionDescriptions = content.match(
      /(他|她|他们)[^\s，]{0,5}(说|道|问|答|喊|叫|笑|叹气|皱眉|摇头|点头|转身)[^\s，。]{0,20}[。]/g
    ) || [];
  ...
  private checkRedundancy(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
  ...
  private checkClarity(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
  ...
  private checkWordChoice(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    let score = 75;
    if (issues.length > 0) score -= issues.length * 7;
    if (uniqueWordRatio > 0.5) score += 5;
  ...
  private checkSentenceVariation(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
  ...
  private checkGenreConvention(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    let score = 70;
    if (issues.length > 0) score -= issues.length * 8;
    if (detectedGenres.length > 0) score += 10;
  ...
  private checkTargetAudience(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    if (detectedAges.includes('children')) targetAudience = '儿童';
    else if (detectedAges.includes('teen')) targetAudience = '青少年';
    else if (detectedAges.includes('adult')) targetAudience = '成年';
    else if (detectedAges.includes('mature')) targetAudience = '成熟';
    
    let score = 70;
    if (issues.length > 0) score -= issues.length * 6;
    if (detectedAges.length > 0) score += 10;
  ...
  private checkAIDetection(content: string): { score: number; status: 'pass' | 'warning' | 'fail'; details: string } {
    const issues: string[] = [];
    ...
    for (const pattern of aiPatterns.overStructure) {
      const matches = content.match(pattern);
      if (matches) overStructureCount += matches.length;
    }
  ...
  private detectRepeatedPhrases(content: string): string[] {
  ...
  private detectTopicShifts(paragraphs: string[]): number {
    const topics: string[] = paragraphs.map(p => {
  ...
  private detectTenseInconsistencies(content: string): string[] {
  ...
  private checkPronounReferences(content: string): string[] {
    const issues: string[] = [];
    const sentences = content.split(/[。！？]/).filter(s => s.trim().length > 0);
  ...
  private detectConsecutiveSameSpeaker(dialogues: string[]): number {
    let count = 0;
  ...
  private detectUnexplainedEvents(content: string): string[] {
    const events: string[] = [];
    const patterns = [
      /突然[^\s，]+出现/g,
      /突然[^\s，]+消失/g,
  ...
  private detectCharacterInconsistencies(content: string): string[] {
    return [];
  }

  private detectMotivationGaps(content: string): number {
  ...
  private detectAbilityInconsistencies(content: string): number {
    return 0;
  }

  private detectImpossibleSequences(content: string): number {
    let count = 0;
    
    const beforeAfter = content.match(/之前[^\n]+之前[^\n]+/g);
    if (beforeAfter) count += beforeAfter.length;
    
    const futurePast = content.match(/将来[^\n]+过去[^\n]+/g);
    if (futurePast) count += futurePast.length;
    
    return count;
  }

  private checkDurationConsistency(patterns: RegExpMatchArray | string[]): boolean {
    return true;
  }

  private checkWorldConsistency(content: string): boolean {
  ...
  private checkActionMotivationPairs(content: string): number {
  ...
  private checkThematicDevelopment(content: string, themes: string[]): boolean {
    if (themes.length === 0) return true;
    
    const paragraphs = content.split(/\n\n+/).filter(p => p.trim().length > 0);
    const themeAppearances = new Map<string, number[]>();
    ...
    for (const [_, appearances] of themeAppearances) {
      if (appearances.length < 2) return false;
  ...
  private detectRepeatedSymbols(content: string): string[] {
  ...
  private assessProseRhythm(content: string): number {
    const sentences = content.split(/[。！？]/).filter(s => s.trim().length > 0);
    const lengths = sentences.map(s => s.length);
    ...
    let score = 50;
    if (coeffVar > 0.3 && coeffVar < 0.7) score += 30;
    else if (coeffVar > 0.2 && coeffVar < 0.8) score += 15;
    
    const punctuationCount = (content.match(/[，。；：]/g) || []).length;
    const punctPerSentence = punctuationCount / Math.max(sentences.length, 1);
    if (punctPerSentence > 2 && punctPerSentence < 8) score += 20;
  ...
  private countSensoryDetails(content: string): number {
    const sensoryWords = [
      '看', '听', '闻', '尝', '触摸', '感受',
      '温暖', '凉爽', '炎热', '寒冷', '光滑', '粗糙',
      '甜蜜', '苦涩', '刺耳', '悦耳', '明亮', '昏暗'
  ...
  private identifyTensionBuilding(paragraphs: string[]): number[] {
    const tensionPhrases = ['突然', '紧张', '危险', '不安', '悬念', '然而', '就在这时'];
    
    const tensionLevels: number[] = [];
  ...
  private assessLanguageComplexity(content: string): number {
    const sentences = content.split(/[。！？]/).filter(s => s.trim().length > 0);
    const words = content.split(/\s+/);
  ...
  private assessDialogueComplexity(content: string): number {
    const dialogues = this.extractDialogues(content);
  ...
  private checkGenreSpecificPatterns(content: string, genres: string[]): { missing: number } {
