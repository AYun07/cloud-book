...
class ContextManager {
     ...
     * 构建写作上下文
     */
    buildWritingContext(project, currentChapter, truthFiles) {
        const contextParts = [];
        // 1. 项目元信息
...
${truthFiles.particleLedger.slice(0, 5).map((r) => `- ${r.name}: ${r.type}`).join('\n')}`;
     ...
     * 构建前文上下文
     */
    buildPreviousContext(project, currentChapter) {
        const previousChapters = project.chapters?.filter(c => c.number < currentChapter && c.summary).slice(-3) || [];
...
${ch.summary || '（无摘要）'}`).join('\n\n')}`;
     ...
     * @DSL 上下文注入 - 完整实现
     */
    parseDSL(text, project, truthFiles) {
        let result = text;
     ...
     * @self - 当前章节引用
     */
    parseSelfReference(text, project, truthFiles) {
     ...
     */
    parseCharacterReference(text, project) {
        let result = text;
        result = result.replace(/@character:(\w+)/g, (match, name) => {
            const char = project.characters?.find(c => c.name.includes(name) || c.aliases?.some(a => a.includes(name)));
            if (!char)
                return match;
     ...
     * @location 或 @loc - 位置引用
     */
    parseLocationReference(text, truthFiles) {
     ...
     */
    parseWorldReference(text, project) {
     ...
     * @chapter:N - 指定章节引用
     */
    parseChapterReference(text, project) {
     ...
     * @hooks 或 @hook:N - 伏笔引用
     */
    parseHookReference(text, truthFiles) {
        const pending = truthFiles.pendingHooks?.filter((h) => h.status !== 'paid_off') || [];
        return text.replace(/@hooks(?::(\d+))?/g, (match, count) => {
            const limit = count ? parseInt(count) : pending.length;
            return pending.slice(0, limit).map((h) => h.description).join('；') || '暂无伏笔';
        });
    }
    /**
     * @relation:xxx - 关系引用
     */
    parseRelationshipReference(text, truthFiles) {
        const snapshot = truthFiles.currentState?.relationshipSnapshot || {};
        return text.replace(/@relation:(\w+)/g, (match, name) => {
            const relationship = Object.entries(snapshot).find(([key]) => key.includes(name));
            if (relationship) {
     ...
     */
    parseTimelineReference(text, truthFiles) {
        const protagonist = truthFiles.currentState?.protagonist;
     ...
     * @item:N 或 @items - 物品引用
     */
    parseItemReference(text, truthFiles) {
        ...
        return text.replace(/@items(?::(\d+))?/g, (match, count) => {
            ...
            return ledger.slice(0, limit).map((r) => r.name).join('、') || '暂无重要物品';
        });
    }
    /**
     * [filter:type] - 过滤表达式
     */
    parseFilterExpressions(text, project, truthFiles) {
        let result = text;
        result = result.replace(/\[filter:char(?:acter)?\s+(\w+)\]/g, (match, keyword) => {
            const chars = project.characters?.filter(c => c.name.includes(keyword) || c.personality?.includes(keyword));
            return chars?.map(c => c.name).join('、') || '';
        });
        result = result.replace(/\[filter:location\s+(\w+)\]/g, (match, keyword) => {
            const locations = project.worldSetting?.locations?.filter(l => l.name.includes(keyword));
     ...
     */
    parseConditionalExpressions(text, project, truthFiles) {
        let result = text;
        result = result.replace(/\{if:(\w+)\}\{then:([^}]*)\}(?:\{else:([^}]*)\})?/g, (match, condition, thenText, elseText) => {
            ...
            switch (condition) {
                ...
                case 'hasPendingHooks':
                    isTrue = (truthFiles.pendingHooks?.filter((h) => h.status !== 'paid_off').length || 0) > 0;
                    break;
                case 'isFirstChapter':
     ...
     * @type:xxx - 按类型获取
     */
    parseTypeReference(text, project, truthFiles) {
        let result = text;
        result = result.replace(/@type:(\w+)/g, (match, type) => {
            switch (type) {
                ...
                case '物品':
                case 'items':
                    return truthFiles.particleLedger?.map(r => r.name).join('、') || '';
                case '章节':
                case 'chapters':
