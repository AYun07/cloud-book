...
export type Locale =
  ...
  | 'es-ES' | 'fr-FR' | 'de-DE' | 'it-IT' | 'pt-BR' | 'pt-PT' | 'ru-RU'
  | 'ar-SA' | 'hi-IN' | 'th-TH' | 'vi-VN' | 'id-ID' | 'ms-MY' | 'tl-PH'
  | 'uk-UA' | 'pl-PL' | 'cs-CZ' | 'sk-SK' | 'hu-HU' | 'ro-RO' | 'bg-BR' | 'el-GR'
  | 'tr-TR' | 'he-IL' | 'fa-IR' | 'ur-PK' | 'bn-BD' | 'ta-IN' | 'ml-IN'
  | 'kn-IN' | 'te-IN' | 'mr-IN' | 'ne-NP' | 'si-LK' | 'my-MM' | 'km-KH'
  | 'lo-LA' | 'zh-SG';
...
export class I18nManager {
  ...
  private pluralRules: Map<Locale, (n: number) => number> = new Map();

  private localeInfo: Record<Locale, LocaleInfo> = {
  ...
  private translationsData: Partial<Record<Locale, TranslationMap>> = {
    'en-US': {
  ...
  private getNestedValue(locale: Locale, key: string): string | TranslationMap | undefined {
    const translations = this.translations.get(locale);
    if (!translations) return undefined;

    const keys = key.split('.');
    let value: string | TranslationMap | undefined = translations;
  ...
  formatRelativeTime(date: Date | string): string {
    ...
    if (minutes < 60) return this.tp('time.minutesAgo', minutes, { count: minutes });
    if (hours < 24) return this.tp('time.hoursAgo', hours, { count: hours });
    if (days < 30) return this.tp('time.daysAgo', days, { count: days });
  ...
  private savePreference(locale: Locale): void {
    try {
  ...
  loadPreference(): void {
    try {
      if (typeof localStorage !== 'undefined') {
        const saved = localStorage.getItem('cloudbook_locale') as Locale;
  ...
  getFullyTranslatedLanguages(): Locale[] {
    const fullyTranslated: Locale[] = [];
    for (const locale of Object.keys(this.translationsData)) {
      if (this.translationsData[locale as Locale]) {
        fullyTranslated.push(locale as Locale);
      }
    }
