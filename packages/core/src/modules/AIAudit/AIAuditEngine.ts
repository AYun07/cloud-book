...
export interface LLMProvider {
  generate(prompt: string, options?: any): Promise<{ text: string; usage?: any }>;
...
export class AIAuditEngine {
  ...
  private async performSemanticAudit(
    content: string,
    truthFiles: TruthFiles,
    context?: { previousChapter?: string; chapterNumber?: number; genre?: string }
...
请对每个维度单独输出一个JSON对象，用---分隔。`;
   ...
   * 构建世界观上下文
   */
  private buildTruthContext(truthFiles: TruthFiles): {
    worldSetting: string;
    characters: string;
    timeline: string;
    hooks: string;
   ...
   * 获取维度描述
   */
  private getDimensionDescriptions(dimensions: string[]): string {
   ...
   * 解析LLM返回的语义审计结果
   */
  private parseSemanticResponse(response: string, dimensions: string[]): SemanticAuditResult[] {
    const results: SemanticAuditResult[] = [];
   ...
   * 基于规则的审计（当LLM不可用时）
   */
  private async performRuleBasedAudit(
    content: string,
    truthFiles: TruthFiles
  ...
  private checkCharacterConsistencyRule(
    content: string, 
    truthFiles: TruthFiles
  ): { dimension: AuditDimension; issues: Issue[] } {
    const issues: Issue[] = [];
    let score = 1.0;
  ...
  private checkRepetitivenessRule(content: string): { dimension: AuditDimension; issues: Issue[] } {
    const issues: Issue[] = [];
  ...
  private checkGrammaticalErrorsRule(content: string): { dimension: AuditDimension; issues: Issue[] } {
    const issues: Issue[] = [];
  ...
  private checkEmotionalArcRule(content: string): { dimension: AuditDimension; issues: Issue[] } {
  ...
  private checkNarrativePacingRule(content: string): { dimension: AuditDimension; issues: Issue[] } {
  ...
  private determineSeverity(score: number, issue: string): 'critical' | 'warning' | 'info' {
