...
class TruthFileManager {
    ...
    validateChapterCharacters(chapter, characters, existingSummaries) {
        const issues = [];
    ...
    validateChapterHooks(chapter, allPendingHooks) {
        const issues = [];
        ...
        for (const hook of chapterHooks) {
            ...
            const laterHook = allPendingHooks.find(h => h.id !== hook.id &&
        ...
        for (const hook of payoffHooks) {
            const correspondingSetHook = allPendingHooks.find(h => h.id === hook.id && h.setInChapter < chapter.number);
    ...
    validateChapterTimeline(chapter, truthFiles) {
        const issues = [];
        if (chapter.timeline) {
    ...
    validateChapterParticleConsistency(chapter, particles) {
        const issues = [];
        if (!chapter.content || chapter.content.trim().length === 0) {
            return issues;
        }
        ...
        for (const itemName of mentionedItems) {
            const item = particles.find(p => p.name.toLowerCase().includes(itemName.toLowerCase()) ||
    ...
    validateProjectHooks(chapters, truthFiles) {
        const issues = [];
    ...
    validateProjectTimeline(chapters, worldTimeline) {
        const issues = [];
    ...
    validateProjectCharacterConsistency(chapters, characters) {
        const issues = [];
        ...
        for (let i = 0; i < sortedFirstAppearances.length; i++) {
            ...
            if (char1?.background) {
                ...
                for (const mentioned of mentionedChars) {
                    const mentionedChar = characters.find(c => c.name.includes(mentioned) || mentioned.includes(c.name));
    ...
    validateProjectResourceFlow(chapters, particles) {
        const issues = [];
