...
class ExportManager {
    ...
    async exportChapter(chapter, project, options) {
        const opts = { ...this.defaultOptions, ...options };
        try {
            let content;
            switch (opts.format) {
    ...
    async exportBatch(chapters, project, options) {
        const results = [];
        let totalSize = 0;
...
<body>
  ...
  <div class="chapter">
    ...
    chapterToTxt(chapter, project, opts) {
        const lines = [];
        if (opts.includeMetadata) {
            lines.push(project.title);
            lines.push('');
    ...
    chapterToMarkdown(chapter, project, opts) {
        const lines = [];
        if (opts.includeMetadata) {
            lines.push(`# ${project.title}`);
            lines.push('');
