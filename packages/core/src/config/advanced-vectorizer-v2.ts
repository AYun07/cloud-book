...
export class AdvancedVectorizerV2 {
  private dimensions: number;
  private ngramMin: number;
  private ngramMax: number;
  private useWeighting: boolean;
  private useSemanticFeatures: boolean;
   ...
   * 搜索（带重排序）
   */
  search(query: string, documents: string[], topK: number = 10): Array<{ index: number; score: number; text: string }> {
...
export const advancedVectorizerV2 = new AdvancedVectorizerV2();
advancedVectorizerV2.fit(PRETRAIN_DOCUMENTS);
