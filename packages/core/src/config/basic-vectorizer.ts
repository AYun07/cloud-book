...
export class BasicVectorizer {
  private dimensions: number;
  private vocabulary: Map<string, number> = new Map();
  private idf: Map<string, number> = new Map();
  private documentCount: number = 0;

  constructor(config: BasicVectorConfig = { dimensions: 1536, method: 'hash' }) {
   ...
   * 搜索最相似
   */
  search(query: string, documents: string[], topK: number = 5): Array<{ index: number; score: number; text: string }> {
...
export const basicVectorizer = new BasicVectorizer();
