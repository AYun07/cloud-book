...
export class TruthFileManager {
   ...
   * 世界状态高级操作 - 添加已知事实
   */
  async addKnownFact(projectId: string, fact: string, chapterNumber: number): Promise<void> {
   ...
   * 世界状态高级操作 - 添加冲突
   */
  async addConflict(projectId: string, conflict: string, chapterNumber: number): Promise<void> {
    const truthFiles = await this.getTruthFiles(projectId);
   ...
   * 世界状态高级操作 - 更新关系快照
   */
  async updateRelationship(projectId: string, characterName: string, status: string): Promise<void> {
    const truthFiles = await this.getTruthFiles(projectId);
   ...
   * 世界状态高级操作 - 批量更新主角状态
   */
  async updateProtagonistState(projectId: string, updates: Partial<ProtagonistState>): Promise<void> {
    const truthFiles = await this.getTruthFiles(projectId);
   ...
   * 世界状态高级操作 - 获取时间线摘要
   */
  async getWorldStateSummary(projectId: string, chapterNumber?: number): Promise<{
   ...
   * 资源账本高级操作 - 批量添加资源
   */
  async addResources(projectId: string, resources: Resource[]): Promise<void> {
    const truthFiles = await this.getTruthFiles(projectId);
   ...
   * 资源账本高级操作 - 更新资源所有者
   */
  async transferResource(projectId: string, resourceId: string, newOwner: string, chapterNumber: number): Promise<void> {
    const truthFiles = await this.getTruthFiles(projectId);
    const resource = truthFiles.particleLedger.find(r => r.id === resourceId);
   ...
   * 资源账本高级操作 - 获取资源清单
   */
  async getResourcesByOwner(projectId: string, owner: string): Promise<Resource[]> {
    const truthFiles = await this.getTruthFiles(projectId);
   ...
   * 资源账本高级操作 - 获取资源变更历史
   */
  async getResourceChangeLog(projectId: string, resourceId: string): Promise<ParticleChange[]> {
    const truthFiles = await this.getTruthFiles(projectId);
    const resource = truthFiles.particleLedger.find(r => r.id === resourceId);
   ...
   * 伏笔钩子高级操作 - 批量添加伏笔
   */
  async addHooks(projectId: string, hooks: Hook[]): Promise<void> {
    const truthFiles = await this.getTruthFiles(projectId);
   ...
   * 伏笔钩子高级操作 - 获取待回收的伏笔
   */
  async getPendingHooks(projectId: string, chapterNumber?: number): Promise<Hook[]> {
    const truthFiles = await this.getTruthFiles(projectId);
   ...
   * 伏笔钩子高级操作 - 获取过期未回收的伏笔
   */
  async getExpiredHooks(projectId: string, currentChapter: number, maxGap: number = 30): Promise<Hook[]> {
    const truthFiles = await this.getTruthFiles(projectId);
    return truthFiles.pendingHooks.filter(h => 
      h.status === 'pending' && 
   ...
   * 伏笔钩子高级操作 - 获取某章节的伏笔
   */
  async getHooksByChapter(projectId: string, chapterNumber: number): Promise<{
    setInChapter: Hook[];
   ...
   * 章节摘要高级操作 - 获取章节范围的摘要
   */
  async getChapterSummariesRange(projectId: string, startChapter: number, endChapter: number): Promise<ChapterSummary[]> {
    const truthFiles = await this.getTruthFiles(projectId);
   ...
   * 章节摘要高级操作 - 获取最近N章摘要
   */
  async getRecentChapterSummaries(projectId: string, count: number = 5): Promise<ChapterSummary[]> {
    const truthFiles = await this.getTruthFiles(projectId);
    return truthFiles.chapterSummaries
   ...
   * 章节摘要高级操作 - 搜索章节摘要
   */
  async searchChapterSummaries(projectId: string, keyword: string): Promise<ChapterSummary[]> {
    const truthFiles = await this.getTruthFiles(projectId);
    const lowerKeyword = keyword.toLowerCase();
    return truthFiles.chapterSummaries.filter(s => 
   ...
   * 管理支线故事的状态和进度
   */
  async addSubplot(projectId: string, subplot: Subplot): Promise<void> {
    const truthFiles = await this.getTruthFiles(projectId);
   ...
   * 支线进度高级操作 - 更新支线状态
   */
  async updateSubplotStatus(projectId: string, subplotId: string, status: Subplot['status'], chapterNumber: number): Promise<void> {
    const truthFiles = await this.getTruthFiles(projectId);
   ...
   * 支线进度高级操作 - 获取活跃支线
   */
  async getActiveSubplots(projectId: string): Promise<Subplot[]> {
    const truthFiles = await this.getTruthFiles(projectId);
   ...
   * 支线进度高级操作 - 获取某章节涉及的支线
   */
  async getSubplotsByChapter(projectId: string, chapterNumber: number): Promise<Subplot[]> {
    const truthFiles = await this.getTruthFiles(projectId);
   ...
   * 支线进度高级操作 - 获取支线进度报告
   */
  async getSubplotProgressReport(projectId: string): Promise<{
    total: number;
    active: number;
    resolved: number;
    abandoned: number;
   ...
   * 情感弧线高级操作 - 批量添加情感点
   */
  async addEmotionPoints(projectId: string, characterId: string, points: EmotionPoint[]): Promise<void> {
    const truthFiles = await this.getTruthFiles(projectId);
    let arc = truthFiles.emotionalArcs.find(a => a.characterId === characterId);
   ...
   * 情感弧线高级操作 - 获取角色情感弧线
   */
  async getEmotionalArc(projectId: string, characterId: string): Promise<EmotionalArc | undefined> {
    const truthFiles = await this.getTruthFiles(projectId);
   ...
   * 情感弧线高级操作 - 获取章节情感快照
   */
  async getChapterEmotions(projectId: string, chapterNumber: number): Promise<EmotionPoint[]> {
    const truthFiles = await this.getTruthFiles(projectId);
    return truthFiles.emotionalArcs.flatMap(arc => 
      arc.points.filter(p => p.chapter === chapterNumber)
    );
  }
   ...
   * 情感弧线高级操作 - 分析情感趋势
   */
  async analyzeEmotionalTrend(projectId: string, characterId: string, recentChapters: number = 10): Promise<{
    ...
    if (recentPoints.length >= 3) {
      ...
      if (lastAvg - firstAvg > 2) trend = 'rising';
      else if (firstAvg - lastAvg > 2) trend = 'falling';
      else {
   ...
   * 角色矩阵高级操作 - 批量添加角色互动
   */
  async addCharacterInteractions(projectId: string, interactions: Array<{
    characterId1: string;
    characterId2: string;
    chapterNumber: number;
    type: string;
    ...
    for (const interactionData of interactions) {
      let interaction = truthFiles.characterMatrix.find(
        i => (i.characterId1 === interactionData.characterId1 && i.characterId2 === interactionData.characterId2) ||
   ...
   */
  async getCharacterInteractions(projectId: string, characterId1: string, characterId2: string): Promise<Interaction[]> {
    const truthFiles = await this.getTruthFiles(projectId);
    const matrix = truthFiles.characterMatrix.find(
      i => (i.characterId1 === characterId1 && i.characterId2 === characterId2) ||
           (i.characterId1 === characterId2 && i.characterId2 === characterId1)
    );
   ...
   * 角色矩阵高级操作 - 获取角色的所有互动
   */
  async getAllCharacterInteractions(projectId: string, characterId: string): Promise<CharacterInteraction[]> {
    const truthFiles = await this.getTruthFiles(projectId);
    return truthFiles.characterMatrix.filter(
      i => i.characterId1 === characterId || i.characterId2 === characterId
    );
  }
   ...
   * 角色矩阵高级操作 - 获取章节角色互动
   */
  async getChapterInteractions(projectId: string, chapterNumber: number): Promise<Array<{
    characterId1: string;
    characterId2: string;
   ...
   * 角色矩阵高级操作 - 分析角色关系网络
   */
  async analyzeRelationshipNetwork(projectId: string): Promise<{
    characters: string[];
    relationships: Array<{
      character1: string;
      character2: string;
   ...
   * 综合功能 - 生成7个真相文件的完整报告
   */
  async generateTruthFilesReport(projectId: string): Promise<{
    worldState: {
      protagonist: any;
    ...
    const totalInteractions = truthFiles.characterMatrix.reduce(
      (sum, m) => sum + m.interactions.length, 0
    );
