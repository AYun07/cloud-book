...
class AgentSystem {
    ...
    registerTools() {
        ...
        this.tools.set('call_agent', {
            ...
            execute: async (params, context) => {
                const result = await this.delegateToAgent(params.agentType, params.task, params.context || {}, context);
                return result;
            }
        });
    ...
    storeAgentMemory(agentType, content, type, importance, tags) {
    ...
    consolidateShortTermMemory(memories) {
    ...
    searchAgentMemory(agentType, query, memoryType, limit = 5) {
        const memKey = typeof agentType === 'number' ? this.getCurrentAgentType() : agentType;
        if (!memKey)
            return [];
        const agentMem = this.agentMemories.get(memKey);
        ...
        const scored = allMemories
            .filter(mem => {
            if (memoryType && mem.type !== memoryType)
                return false;
    ...
    extractTruthInfo(truthFiles, checkType) {
        switch (checkType) {
            case 'protagonist':
    ...
    updateTruthFiles(updateType, data, context) {
        if (!context.truthFiles)
            return;
        switch (updateType) {
            case 'protagonist':
            ...
            case 'add_hook':
                ...
                break;
            case 'resolve_hook':
                const hookIndex = context.truthFiles.pendingHooks.findIndex((h) => h.id === data.id);
                if (hookIndex >= 0) {
            ...
            case 'update_resource':
                const resourceIndex = context.truthFiles.particleLedger.findIndex((r) => r.id === data.id);
                if (resourceIndex >= 0) {
    ...
    async analyzeContentAspect(aspect, content, context) {
        const aspectPrompts = {
...
请提供详细的分析结果，包括发现和洞察。`;
    ...
    async planNextActions(currentTask, constraints, context) {
...
以 JSON 格式输出。`;
    ...
    async evaluateDecisionOptions(options, criteria, context) {
        const prompt = `请评估以下选项并给出推荐：

选项：
${options.map((opt, i) => `${i + 1}. ${typeof opt === 'string' ? opt : JSON.stringify(opt)}`).join('\n')}
...
请对每个选项进行评分（0-10），并说明推荐理由。以 JSON 格式输出：
{
  ...
  "evaluations": [{ "option": 索引, "score": 分数, "reasoning": "理由" }]
        ...
        return { recommended: 0, evaluations: options.map((_, i) => ({ option: i, score: 5 })) };
    }
    async think(agentType, task, context, maxIterations = 3) {
        const thinkingSteps = [];
        let currentContext = { ...context };
        let iteration = 0;
        while (iteration < maxIterations) {
            const step = { thought: '', reasoning: '' };
...
请用 JSON 格式输出：
{
  ...
  "toolCalls": [{"tool": "工具名", "params": {"参数"}}] // 如果需要工具调用
    ...
    async delegateToAgent(targetType, task, taskContext, senderContext) {
        const message = {
            id: `msg-${Date.now()}-${Math.random().toString(36).substr(2, 6)}`,
            from: senderContext.currentChapter || 'writer',
            to: targetType,
            type: 'request',
    ...
    broadcastMessage(agentType, type, content, excludeSelf) {
        const message = {
            id: `msg-${Date.now()}-${Math.random().toString(36).substr(2, 6)}`,
    ...
    async reflect(agentType, trigger) {
...
以 JSON 格式输出：
{
  ...
  "adjustments": ["调整1", "调整2"]
    ...
    async executeArchitectTaskWithAgent(project, task, params) {
        const agentType = 'architect';
    ...
    async executeWriterTaskWithAgent(project, chapterNumber, options, context) {
        const agentType = 'writer';
        ...
        try {
            ...
            const thinkingSteps = await this.think(agentType, `为小说项目 ${project.title} 写作第 ${chapterNumber} 章`, agentContext);
    ...
    async executeAuditorTaskWithAgent(content, truthFiles, options, context) {
        const agentType = 'auditor';
        this.agentStates.get(agentType).status = 'executing';
        try {
            const agentContext = context || {
                memories: this.agentMemories.get(agentType),
    ...
    async executeReviserTaskWithAgent(content, issues, truthFiles, context) {
        const agentType = 'reviser';
        this.agentStates.get(agentType).status = 'executing';
        try {
            const agentContext = context || {
                memories: this.agentMemories.get(agentType),
    ...
    async executeStyleEngineerTaskWithAgent(content, task, styleSource, context) {
        const agentType = 'styleEngineer';
        this.agentStates.get(agentType).status = 'executing';
        try {
            const agentContext = context || {
                memories: this.agentMemories.get(agentType),
    ...
    async executeRadarTaskWithAgent(task, params) {
        const agentType = 'radar';
        this.agentStates.get(agentType).status = 'executing';
        try {
            const agentContext = {
                memories: this.agentMemories.get(agentType),
...
请确保节奏感：前期铺垫、中期发展、后期高潮。`;
    ...
    buildWriterPrompt(project, chapterNumber, options, agentContext) {
...
请开始写作：`;
    ...
    buildReviserPromptWithAgent(content, issues, truthFiles, context) {
        const criticalIssues = issues.filter((i) => i.severity === 'critical');
        const warnings = issues.filter((i) => i.severity === 'warning');
        return `请根据以下问题修订章节内容，注意保持与已建立世界观和角色设定的一致性。

【必须修复的问题】（严重）：
${criticalIssues.map((i, idx) => `${idx + 1}. [${i.type}] ${i.description} - 建议：${i.suggestion || '请自行修复'}`).join('\n')}

【建议优化的问题】：
${warnings.map((i, idx) => `${idx + 1}. [${i.type}] ${i.description}`).join('\n')}
...
【待回收的伏笔】：
${truthFiles.pendingHooks?.length ? truthFiles.pendingHooks.map((h) => `- ${h.description}`).join('\n') : '无'}
